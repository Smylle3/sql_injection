...
//// Endpoint vulnerável - remoção de projetos
//app.delete('/projects/delete', (req, res) => {
//    if (req.user) {
//		const projectToRemove = projects.findIndex(p => p.id === req.query.deleteId)
//		console.log(projectToRemove, req.query.deleteId);
//        projects.splice(projectToRemove, 1);
//        res.send(`Projeto ${req.query.deleteId} deletado com sucesso!`);
//    } else {
//        res.status(403).send('Acesso negado.');
//    }
//});
...
// Endpoint vulnerável - remoção de projetos
app.delete('/projects/delete', checkManager, (req, res) => {
	const projectToRemove = projects.findIndex(p => p.id === req.query.deleteId)
	if(projectToRemove === -1) {
		res.status(404).send(`Projeto ${req.query.deleteId} não foi encontrado!`)
	} else {
		projects.splice(projectToRemove, 1);
		res.send(`Projeto ${req.query.deleteId} deletado com sucesso!`);
	}
